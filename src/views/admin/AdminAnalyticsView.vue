<template>
  <div class="space-y-lg">
    ...
    <div v-if="loading" class="space-y-lg">
      <div class="bg-gradient-to-r from-primary/10 to-surface-container border border-primary/20 p-lg animate-pulse">
        <div class="h-20 bg-primary/10"></div>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-md">
        <div v-for="n in 4" :key="n" class="bg-surface border border-outline-variant/30 p-md animate-pulse h-24"></div>
      </div>
    </div>
      ...
      <div class="bg-gradient-to-r from-primary/10 to-surface-container border border-primary/20 p-lg shadow-sm">
        <div class="flex items-start gap-md">
          ...
          <div class="space-y-xs">
            <p class="font-headline-md text-sm text-primary font-semibold uppercase tracking-widest">Resumen Inteligente &mdash; Junio 2026</p>
            <p class="font-body-md text-sm text-on-surface">
              Los ingresos del mes son <strong class="text-primary">$18,420</strong>, un <strong class="text-success">+8%</strong> respecto al mes anterior.
              El servicio m&aacute;s solicitado fue <strong class="text-primary">Balayage Premium</strong>. Los martes y viernes concentran el <strong class="text-primary">62%</strong> de las citas.
              Se recomienda abrir disponibilidad adicional los <strong class="text-primary">mi&eacute;rcoles</strong> con Elena Vargas para satisfacer la demanda reprimida.
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-md">
        <div v-for="stat in stats" :key="stat.label" class="bg-surface border border-outline-variant/30 p-md shadow-sm">
          <div class="flex items-center justify-between mb-sm">
            <span class="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-widest">{{ stat.label }}</span>
        ...
        <div class="bg-surface border border-outline-variant/30 p-md shadow-sm space-y-sm">
          ...
          <div v-for="(spec, idx) in topSpecialists" :key="spec.name" class="flex items-center gap-md py-sm border-b border-outline-variant/10 last:border-0">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-label-sm text-on-surface-variant">{{ spec.specialty }}</p>
            </div>
            <div class="text-right">
              <p class="font-label-md text-sm font-bold text-primary">{{ spec.bookings }}</p>
              <p class="text-label-sm text-on-surface-variant">citas</p>
            </div>
...
const stats = shallowRef<StatCard[]>([]);
const servicePerf = shallowRef<ServicePerf[]>([]);
const topSpecialists = shallowRef<TopSpecialist[]>([]);

onMounted(async () => {
  const startOfMonth = new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0];

  const [{ count: completedCount }, { count: cancelledCount }, { count: totalCount }, { data: feedbackData }] = await Promise.all([
    supabase.from('appointments').select('*', { count: 'exact', head: true }).eq('status', 'Completada'),
    supabase.from('appointments').select('*', { count: 'exact', head: true }).eq('status', 'Cancelada'),
    supabase.from('appointments').select('*', { count: 'exact', head: true }),
    supabase.from('feedback').select('rating')
  ]);

  const avgRating = feedbackData?.length
    ? (feedbackData.reduce((s: number, f: any) => s + f.rating, 0) / feedbackData.length).toFixed(1)
    : '0.0';
  ...
  stats.value = [
    ...
    { label: 'Tasa de Cancelaci&oacute;n', value: `${cancelRate}%`, icon: 'pi pi-times-circle', progress: Math.min(Number(cancelRate) * 10, 100) },
    { label: 'Satisfacci&oacute;n Promedio', value: `${avgRating} \u2605`, icon: 'pi pi-star-fill', progress: Math.round(Number(avgRating) * 20) },
  ...
  topSpecialists.value = (specs || []).map((s: any) => ({
    name: s.name,

  ...
  <div class="space-y-md">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-sm bg-surface border border-outline-variant/30 px-md py-sm shadow-sm">
        <div class="text-center">
          ...
          <p class="text-[10px] text-on-surface-variant mt-xs">{{ reviews.length }} valoraciones</p>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-md">
      <div class="bg-surface border border-outline-variant/30 p-md shadow-sm md:col-span-1">
        <h3 class="font-label-md text-sm text-on-surface-variant uppercase tracking-widest mb-md">Distribución de Calificaciones</h3>
      ...
      <div class="md:col-span-2 bg-surface border border-outline-variant/30 p-md shadow-sm">
        <div class="flex items-center justify-between mb-md">
          ...
          <app-dropdown id="filterRating" v-model="filterRating" label="Calificación" :options="ratingOptions" optionLabel="label" optionValue="value" class="w-40" />
        </div>
        <div class="space-y-sm max-h-[360px] overflow-y-auto pr-xs">
          <div v-for="review in filteredReviews" :key="review.id"
            ...
            <div class="flex items-center justify-between mt-sm">
              <span class="flex items-center gap-xs text-[10px] text-primary font-medium">
                <i class="pi pi-tag"></i> {{ review.serviceName }}
              </span>
              <button class="text-[10px] text-on-surface-variant hover:text-primary flex items-center gap-xs transition-colors">
...
const countByRating = (n) => reviews.value.filter(r => r.rating === n).length;
const ratingPercent = (n) => reviews.value.length > 0 ? (countByRating(n) / reviews.value.length) * 100 : 0;
